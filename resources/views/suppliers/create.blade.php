...
@section('title', 'Add supplier · Price Hub')

@section('content')
    <div class="flex items-center justify-between mb-3">
        <div>
            <h1 class="text-lg font-semibold tracking-tight">Add supplier</h1>
            <p class="text-xs text-slate-500">Створення нового джерела прайс-листів.</p>
        </div>
    ...
    <form action="{{ route('suppliers.store') }}" method="POST"
        ...
        <div class="grid md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="type"
                        class="w-full border rounded-lg px-3 py-2 text-xs">
                    <option value="http"  @selected(old('type') === 'http')>HTTP (direct link)</option>
                    <option value="ftp"   @selected(old('type') === 'ftp')>FTP</option>
                    <option value="gdrive"@selected(old('type') === 'gdrive')>Google Drive</option>
                </select>
            </div>
        ...
        <div class="grid md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="ext" class="w-full border rounded-lg px-3 py-2 text-xs">
                    ...
                    <option value="csv"  @selected(old('ext') === 'csv')>CSV</option>
                    <option value="xls"  @selected(old('ext') === 'xls')>XLS</option>
                    <option value="xlsx" @selected(old('ext') === 'xlsx')>XLSX</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium mb-1">Header row</label>
                <input type="number" name="header_row" value="{{ old('header_row', 1) }}"
                       class="w-full border rounded-lg px-3 py-2 text-xs">
            </div>
            <div>
                <label class="block text-xs font-medium mb-1">First data row</label>
                <input type="number" name="start_row" value="{{ old('start_row', 2) }}"
                       class="w-full border rounded-lg px-3 py-2 text-xs">
            </div>
        </div>
        ...
        <div class="border-t pt-3 mt-1">
            <h2 class="text-xs font-semibold text-slate-600 mb-2">
                Optional: initial column mapping (можна налаштувати пізніше в “Map columns”)
            </h2>
